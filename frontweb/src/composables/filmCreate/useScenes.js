...
export function useScenes(deps) {
  ...
  function buildSceneImageMeta(scene) {
    ...
    return {
      ...
      resourceId: scene.id,
      label: `${epLabel} Ảnh scene: ${scene.location || scene.id}`,
    }
  }
  ...
  async function onExtractScenes() {
    ...
    const meta = buildExtractTaskMeta(store, dramaId.value, epId, GEN_RESOURCE.EXTRACT_SCENES, 'Trích xuất scene')
    ...
    try {
      ...
      if (taskId) {
        ...
        if (pollRes?.status !== 'failed') {
          ElMessage.success('Đã trích xuất scene xong')
        }
      } else {
        await loadDrama()
        ElMessage.success('Đã gửi task trích xuất scene')
      }
    } catch (e) {
  ...
  async function doGenerateSceneSinglePrompt() {
    ...
    try {
      ...
      if (res?.polished_prompt_single) {
        ...
        ElMessage.success('Đã tạo prompt ảnh đơn')
        await loadDrama()
      }
    } catch (e) {
      ElMessage.error(e.message || 'Tạo prompt thất bại')
  ...
  async function saveSceneRefImageIfAny(sceneId) {
    ...
    try {
      ...
      console.warn('[saveSceneRefImage] failed to save reference image:', e.message)
    }
  }
  ...
  async function doExtractSceneFromImage() {
    ...
    try {
      ...
      if (res?.prompt) {
        ...
        ElMessage.success('Đã trích xuất mô tả scene từ ảnh')
      }
    } catch (e) {
      ElMessage.error(e.message || 'Trích xuất thất bại, vui lòng kiểm tra xem scene đã có ảnh tham chiếu chưa')
    } finally {
  ...
  async function submitEditScene() {
    ...
    try {
      if (form.id) {
        ...
        ElMessage.success('Đã lưu scene')
      } else {
  ...
  async function onDeleteScene(scene) {
    try {
      await ElMessageBox.confirm(
        `Bạn có chắc muốn xoá scene "${(scene.location || scene.time || 'Chưa đặt tên').slice(0, 20)}"? Thao tác này không thể hoàn tác.`,
        'Xác nhận xoá',
        { type: 'warning', confirmButtonText: 'Xoá', cancelButtonText: 'Huỷ' }
      )
      await sceneAPI.delete(scene.id)
      await loadDrama()
      ElMessage.success('Đã xoá scene')
    } catch (e) {
      if (e === 'cancel') return
  ...
  async function onGenerateSceneImage(scene, useQuadGrid = false) {
    ...
    try {
      ...
      if (taskId) {
        const pollRes = await pollTask(taskId, () => loadDrama(), meta)
        if (pollRes?.status === 'failed') {
          scene.errorMsg = pollRes.error || 'Tạo thất bại'
        } else {
          ElMessage.success('Đã tạo ảnh scene')
        }
      } else {
        await loadDrama()
      ...
      console.error(e)
      scene.errorMsg = e.message || 'Tạo thất bại'
  ...
  async function onDeleteSceneLibrary(item) {
    try {
      ...
      await ElMessageBox.confirm(
        `Bạn có chắc muốn xoá scene công khai "${name}"?`,
        'Xác nhận xoá',
        { type: 'warning', confirmButtonText: 'Xoá', cancelButtonText: 'Huỷ' }
      )
  ...
  async function onAddSceneToLibrary(scene) {
    if (!hasAssetImage(scene)) { ElMessage.warning('Vui lòng tạo hoặc tải lên ảnh cho scene này trước'); return }
    addingSceneToLibraryId.value = scene.id
    try {
      await sceneAPI.addToLibrary(scene.id, {})
      ElMessage.success('Đã thêm vào thư viện scene của phim')
  ...
  async function addSceneToEpisode(item, scope) {
    ...
    try {
      ...
      if (existingScene) {
        ...
        ElMessage.success(`Đã cập nhật "${item.location || 'scene'}" vào tập này`)
      } else {
        await sceneAPI.create({
          drama_id: store.dramaId,
        ...
        ElMessage.success(`Đã thêm "${item.location || 'scene'}" vào tập này`)
      }
      await loadDrama()
    } catch (e) {
      ElMessage.error(e.message || 'Thêm thất bại')
